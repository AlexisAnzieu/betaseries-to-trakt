import {
  fetchMovieIdentifiers,
  fetchShowIdentifiers,
} from './betaseries'
...
export const migrateLibrary = async ({
  ...
  const showWatchlistRows = shows.filter((row) => Number(row.status ?? '0') === 0)
  const showHistoryRows = shows.filter((row) => Number(row.status ?? '0') !== 0)

...
export const TraktDeviceFlow = ({
  ...
  useEffect(() => {
    ...
    if (!clientSecret) {
      setMessage('Provide your Trakt client secret to start polling for approval.')
      return undefined
    }
  ...
  const handleRequestCode = async () => {
    ...
    try {
      ...
      setStatus('waiting')
      setMessage('Visit Trakt, enter the user code and approve the application.')
    } catch (error) {
      setStatus('error')
      setMessage(
    ...
    <section className="panel">
      ...
      <div className="field">
        ...
        <input
          ...
          value={clientId}
          onChange={(event) => onClientIdChange(event.currentTarget.value.trim())}
        />
      </div>
      ...
      >
        {status === 'requesting' ? 'Requesting device code…' : 'Request device code'}
      </button>

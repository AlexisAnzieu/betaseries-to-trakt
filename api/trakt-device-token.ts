...
export default async function handler(
  ...
  const { clientId, clientSecret, deviceCode } = (req.body ?? {}) as DeviceTokenRequestBody

  if (!clientId || !clientSecret || !deviceCode) {
    res.status(400).json({ error: 'Missing clientId, clientSecret, or deviceCode' })
    return
  }

  try {
    const traktResponse = await exchangeTraktDeviceCode(clientId, clientSecret, deviceCode)
    res
